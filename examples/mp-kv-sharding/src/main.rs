...
fn mc_check_query<'a>(
    ...
    node: &'a str,
    proc: &'a str,
    invariant: Box<dyn Fn(usize) -> Box<dyn Fn(&McState) -> Result<(), String> + 'a> + 'a>,
    msg: Message,
...
fn check_mc_get<'a>(
    ...
    let invariant = Box::new(
        move |start_len: usize| -> Box<dyn Fn(&McState) -> Result<(), String> + 'a> {
...
fn test_model_checking_normal(config: &TestConfig) -> TestResult {
    ...
    let achieved_states = check_mc_put(
        ...
        &value,
        max_steps,
        Some(achieved_states.collected),
    )?;
